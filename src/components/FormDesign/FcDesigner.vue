        ...
        <el-aside class="_fc-l" width="266px">
          <template v-for="(item, index) in menuList">
            <div class="_fc-l-group" :key="index">
              ...
              <draggable :group="{name:'default', pull:'clone',put:false}" :sort="false" :list="item.list">
                <div class="_fc-l-item" v-for="(data, index2) in item.list" :key="index2">
        ...
        <ElContainer class="_fc-m">
          <el-header class="_fc-m-tools" height="45">
            ...
            <el-button
              ...
              round
              size="mini"
              @click="clearDragRule">清 空</el-button>
          </el-header>
        ...
        <ElAside class="_fc-r" width="320px">
          <ElContainer style="height: 100%;">
            <el-header height="40px" class="_fc-r-tabs">
              <div class="_fc-r-tab" :class="{active: activeTab==='props'}" v-if="!!activeRule" @click="activeTab='props'">组件配置
              </div>
              <div class="_fc-r-tab" :class="{active: activeTab==='form' && !!activeRule}" @click="activeTab='form'">表单配置
              </div>
            </el-header>
            <ElMain v-show="activeTab==='form'">
              <FormCreate :rule="form.rule" :option="form.option" :value.sync="form.value.form"></FormCreate>
            </ElMain>
            <ElMain v-show="activeTab==='props'" style="padding: 0 20px;" :key="activeRule? activeRule.id: ''">
              <div>
