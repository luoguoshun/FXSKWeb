...
export function getOrderData (pagination, filters) {
    return request({
        url: orderApi.getOrderData + `?departmen=${filters.department}&sendDepartment=${filters.sendDepartment}&keyword=${filters.type}&value=${filters.keys}&startDate=${filters.startDate}&endDate=${filters.endDate}&page=${pagination.current}&limit=${pagination.pageSize}&pathologyType=${filters.pathologyType}&feeIhcFlag=${filters.feeIhcFlag}`,
        method: 'get'
    })
}
// 获取预约申请单列表数据
export function getAppointmentOrderData (pagination, filters) {
    return request({
        url: orderApi.getAppointmentOrderData + `?keyword=${filters.type}&value=${filters.keys}&startDate=${TimeToString(filters.startDate)}&endDate=${TimeToString(filters.endDate)}&page=${pagination.current}&limit=${pagination.pageSize}&pathologyType=${filters.pathologyType}`,
        method: 'get'
    })
}
// 获取申请单列表数据(打印)
...
export function getDoctorTakeAway (pagination, filters) {
    return request({
      url: orderApi.getDoctorTakeAway  + `?code=${filters.code}&startDate=${filters.startDate}&endDate=${filters.endDate}&pageIndex=${pagination.current}&pageSize=${pagination.pageSize}`,
      method: 'get'
    })
}
