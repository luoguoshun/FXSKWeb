...
function BrowserPrintInit (obj) {
    const $this = obj
    BrowserPrint.getDefaultDevice('printer', function (device) {
        $this.GLOBAL.selectedDevice = device
        $this.GLOBAL.devices.push(device)
        BrowserPrint.getLocalDevices(function (deviceList) {
            for (var i = 0; i < deviceList.length; i++) {
                var device = deviceList[i]
                if (!$this.GLOBAL.selectedDevice || device.uid !== $this.GLOBAL.selectedDevice.uid) {
                    $this.GLOBAL.devices.push(device)
                }
            }
        }, function () {
