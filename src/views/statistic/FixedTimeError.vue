<template>
  <div>
    <a-card :bordered="false">
      <a-alert message="通过设置离体和固定时间间隔，从而筛选超过间隔的异常标本信息" banner style="margin-bottom:10px;"/>
...
<script>
import { getSpecimenFixedPager } from '@/api/specimentrace/specimentrace'
...
const columns = [
  ...
  { title: '固定操作人', dataIndex: 'Operator2' },
...
export default {
  ...
  methods: {
    ...
    load (pageIndex, pageSize, parameter) {
      ...
      getSpecimenFixedPager(param).then((res) => {
        this.loading = false
        $this.data = res.data
    ...
    exportFile () {
      ...
      getSpecimenFixedPager(param).then((res) => {
        this.loading = false
        const excelHeader = ['打包科室', '门诊号', '住院号', '申请单号', '患者姓名', '年龄', '录入时间', '标本名','离体操作人','固定操作人', '固定时间', '离体时间', '时间间隔']
        const filterVal = ['PackageDeptName', 'OutpatientNum', 'AdmissionNum', 'AppNo', 'PatientName', 'Age', 'ReqDateTime', 'SpecimenNames', 'Operator', 'Operator2', 'LeaveTime', 'FixedTime', 'IntervalTime']
        res.data.forEach(f => {
          if (CheckField(f.OutpatientNum) === false) {
            f.OutpatientNum = ''
            if (f.ReqDateTime !== undefined) {
