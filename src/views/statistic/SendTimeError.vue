<template>
  <div>
    <a-card :bordered="false">
      <a-alert message="通过设置运送和接收时间间隔，从而筛选超过间隔的异常标本信息" banner style="margin-bottom:10px;"/>
...
<script>
import { getSpecimenSendPager } from '@/api/specimentrace/specimentrace'
...
const columns = [
  ...
  { title: '运送时间', dataIndex: 'SendTime', customRender: (val) => { return val ? moment(val).format('YYYY-MM-DD HH:mm:ss') : '' } },
  ...
  { title: '运送操作人', dataIndex: 'Operator' },
...
export default {
  ...
  methods: {
    ...
    load (pageIndex, pageSize, parameter) {
      ...
      getSpecimenSendPager(param).then((res) => {
        this.loading = false
        $this.data = res.data
    ...
    exportFile () {
      ...
      getSpecimenSendPager(param).then((res) => {
        this.loading = false
        const excelHeader = ['打包科室', '门诊号', '住院号', '申请单号', '患者姓名', '年龄', '录入时间', '标本名','运送操作人','接收操作人', '运送时间', '接收时间', '时间间隔']
        const filterVal = ['PackageDeptName', 'OutpatientNum', 'AdmissionNum', 'AppNo', 'PatientName', 'Age', 'ReqDateTime', 'SpecimenNames', 'Operator', 'Operator2', 'SendTime', 'ReceiveTime', 'IntervalTime']
        res.data.forEach(f => {
        ...
        exportJsonToExcel(excelHeader, data, '标本运送异常信息')
