<template>
  <div ref="No">
    <a-card :bordered="false">
        ...
        <a-col span="24">
          <a-space>
            ...
            <a-input-search
              ...
              enter-button
              @search="onSearch" />
            申请单数：<a-tag style="font-size:25px; height:25px;" color="#2db7f5">{{ appOrderTotal }}</a-tag>
            未确认数：<a-tag style="font-size:25px; height:25px;" color="#f50">{{ notConfirm }}</a-tag>
            已确认数：<a-tag style="font-size:25px; height:25px;" color="#87d068">{{ isConfirm }}</a-tag>
          </a-space>
        </a-col>
      </a-row>
      ...
      <a-list>
        <a-list-item>
          <a-card size="small" style="height:400px;" :title="'未确定标本列表(' + notConfirm + ')'">
            <a-table
              :scroll="{ x: 530, y: 300 }"
              size="small"
              :columns="columnsForNotCheck"
              rowKey="Id"
              :pagination="false"
              :data-source="dataForNotCheck"
          ...
          <a-card size="small" style="height:400px;" :title="'已确定标本列表(' + isConfirm + ')'">
            <a-table
              :scroll="{ x: 920, y: 300 }"
              size="small"
...
const dataForCheck = []// 一级列表数据
const columnsForCheck = [
...
const data = []// 一级列表数据
const dataForNotCheck = []
const columnsForNotCheck = [
  { title: '送检号', dataIndex: 'SendNo', scopedSlots: { customRender: 'SendNo' } },
  { title: '申请单号', dataIndex: 'AppNo' },
  { title: '标本编号', dataIndex: 'Id' },
  { title: '标本名称', dataIndex: 'SpecimenName' },
...
export default {
  ...
  methods: {
          ...
          return new Promise(resolve => {
            ...
            doReceive({ 'ObjectDTO': dto }).then(res => {
              if (res.HttpStatusCode === 200 && res.ResultType === 1) {
                ...
                $this.data = $this.data.filter(f => dto.SpecimenIds.indexOf(f.Id) === -1)
                setTimeout(() => {
                  $this.refreshScanTotal()
                  resolve(true)
                }, 1000)
    ...
    refreshScanTotal (isReload = false) {
      ...
      this.dataForCheck = this.data.filter(f => f.IsConfirm === true)
      this.dataForNotCheck = this.data.filter(f => f.IsConfirm === false)
    }
  }
}
</script>
